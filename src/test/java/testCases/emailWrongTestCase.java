...
public class emailWrongTestCase extends pageBaseClass{
	homePage homePage;
	...
	@Parameters("browser")
	@Test(dependsOnGroups = "testOne", groups = "testEmail")
	public void submitForm(String browser) throws IOException {
		//Took data from Excel sheet using Apache Poi
				...
				String name = formatter.formatCellValue(sheet.getRow(7).getCell(0));
				String organizationName = formatter.formatCellValue(sheet.getRow(7).getCell(1));
				String email = formatter.formatCellValue(sheet.getRow(7).getCell(2));
				String phone = formatter.formatCellValue(sheet.getRow(7).getCell(3));
				String size = formatter.formatCellValue(sheet.getRow(7).getCell(4));
				
				//Closed the workbook
				workbook.close();
				
				//Extent report initialization
				logger = report.createTest("Search for hospitals Test 2");
